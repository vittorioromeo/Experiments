...
inline void printNode(std::ostream& mStream, TN& mValue, std::size_t mDepth)
{
    ...
    {
        callStringifyImpl<TFmt>(
            mStream, mValue.template getAs<Lang::ASTExpr>().getName());
    }
    else if(dynamic_cast<Eng::ASTTokenNode<Lang::Spec>*>(&mValue))
    {
        callStringifyImpl<TFmt>(
            mStream, Lang::tknToStr(
                         mValue.template getAs<Eng::ASTTokenNode<Lang::Spec>>()
                             .getToken()
                             .getType()));
    }
    ...
    parenthesizedExpr.setPredicate([](Ctx<Spec>& mCtx)
        {
            ...
            return mCtx[0].isToken(Tkn::POpen) &&
                   mCtx[1].isDerivedFrom<ASTExpr>() &&
                   mCtx[2].isToken(Tkn::PClose);
        });
...
#define RULE_CREATE_BIN_OP(mName, mTknName, mOpName, mLookAhead)       \
    ...
    mName.setPredicate([](Ctx<Spec>& mCtx)                             \
        {                                                              \
            if(mCtx.getSize() < 3) return false;                       \
            return mCtx[0].isDerivedFrom<ASTExpr>() &&                 \
                   mCtx[1].isToken(Tkn::mTknName) &&                   \
                   mCtx[2].isDerivedFrom<ASTExpr>();                   \
        });                                                            \
    ...
    if(mLookAhead)                                                     \
    mName.setLookAhead([](Ctx<Spec>& mCtx, LookAheadResults& mResults) \
        {                                                              \
            if(!mCtx.canLookAhead(3)) return;                          \
            if(mCtx.getAhead(3).isToken(Tkn::OpMul) ||                 \
                mCtx.getAhead(3).isToken(Tkn::OpDiv) ||                \
                mCtx.getAhead(3).isToken(Tkn::OpMod))                  \
            {                                                          \
    ...
    std::vector<Token<Spec>> src{{Tkn::Number}, {Tkn::OpAdd}, {Tkn::Number},
        {Tkn::OpMul}, {Tkn::Number}, {Tkn::OpAdd}, {Tkn::Number}, {Tkn::OpAdd},
        {Tkn::Number}, {Tkn::OpDiv}, {Tkn::Number}, {Tkn::OpAdd},
        {Tkn::Number}};

    std::vector<Token<Spec>> src2{{Tkn::POpen}, {Tkn::POpen}, {Tkn::Number},
        {Tkn::OpAdd}, {Tkn::Number}, {Tkn::PClose}, {Tkn::OpMul}, {Tkn::POpen},
        {Tkn::Number}, {Tkn::OpAdd}, {Tkn::Number}, {Tkn::PClose},
        {Tkn::PClose}, {Tkn::OpMul}, {Tkn::Number}};

    std::vector<Token<Spec>> src3{{Tkn::POpen}, {Tkn::Number}, {Tkn::OpAdd},
        {Tkn::Number}, {Tkn::PClose}, {Tkn::OpMul}, {Tkn::POpen}, {Tkn::Number},
        {Tkn::OpAdd}, {Tkn::Number}, {Tkn::PClose}};
