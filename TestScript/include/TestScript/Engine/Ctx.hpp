	...
	template<typename TL> class Ctx
	{
		...
		public:
			...
			class Proxy
			{
				...
				public:
					inline Proxy(Ctx<TL>& mCtx, NodePtr<TL> mNodePtr) : ctx(mCtx), nodePtr{mNodePtr} { }
					...
					template<typename T> inline bool isDerivedFrom() const noexcept
					{
		...
		public:
			...
			{
				auto& node(parser.template createNode<T>(SSVU_FWD(mArgs)...));
			...
			inline Proxy getAhead(std::size_t mIdx)
			{
				...
				if(idxWO < parser.parseStack.size()) return Proxy{*this, parser.parseStack[idxWO]};
