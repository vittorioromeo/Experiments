...
namespace Eng
{
    ...
    public:
        inline Rule<TL>& createRule()
        {
            return ssvu::getEmplaceUPtr<Rule<TL>>(rules);
        }
        ...
        inline Node<TL>& createNode(TArgs&&... mArgs)
        {
            auto& result(nodeManager.template create<T>(FWD(mArgs)...));
            nodeManager.refresh();
