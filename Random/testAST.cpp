...
namespace Eng
{
    ...
    namespace Internal
    {
        inline ASTTypeId getNextASTTypeId() noexcept
        {
            static ASTTypeId lastId{0};
            return lastId++;
        }
    }

    template <typename T>
    inline const ASTTypeId& getASTTypeId() noexcept
    {
        static ASTTypeId id{Internal::getNextASTTypeId()};
        return id;
    }
    ...
    template <typename TL>
    using ASTNodePtr = ASTNode<TL>*;
    template <typename TL>
    using ASTImplPtr = ASTImpl<TL>*;
    template <typename TL>
    using ASTNodeUPtr = ssvu::UPtr<ASTNode<TL>>;
    template <typename TL>
    using ASTImplUPtr = ssvu::UPtr<ASTImpl<TL>>;

    template <typename TL, typename T, typename... TArgs>
    inline ASTImplUPtr<TL> createASTImpl(TArgs&&... mArgs);
    ...
    class ASTImpl
    {
        ...
        template <typename TL2, typename T, typename... TArgs>
        friend ASTImplUPtr<TL2> createASTImpl(TArgs&&... mArgs);
    ...
    template <typename TL>
    class ASTTokenNodeImpl : public ASTImpl<TL>
    {
    private:
        Token<TL> token;
    ...
    public:
        inline RulePart(TokenType<TL> mTokenType) noexcept
            : type{RPType::Token},
              tokenType{mTokenType}
        {
        }
        inline RulePart(ASTTypeId mASTTypeId) noexcept : type{RPType::Node},
                                                         astTypeId{mASTTypeId}
        {
        }

        inline RPType getType() const noexcept { return type; }
        inline const decltype(tokenType)& getTokenType() const noexcept
        {
            SSVU_ASSERT(type == RPType::Token);
            return tokenType;
        }
        inline ASTTypeId getASTTypeId() const noexcept
        {
            SSVU_ASSERT(type == RPType::Node);
            return astTypeId;
        }
        ...
        inline bool matchesNode(ASTNodePtr<TL> mNode) const
        {
            if(mNode->getTypeId() == Eng::getASTTypeId<ASTTokenNodeImpl<TL>>())
            {
                ...
                auto& pTokenImpl(
                    mNode->template getImplAs<ASTTokenNodeImpl<TL>>());
        ...
        inline bool operator==(const RulePart<TL>& mRhs) const noexcept
        {
            ...
            if(type == RPType::Token && tokenType == mRhs.tokenType)
                return true;
    ...
    private:
        ...
        template <typename T>
        inline void ctorImpl(T mP)
        {
            ruleParts.emplace_back(mP);
        }
        template <typename T1, typename T2, typename... TArgs>
        inline void ctorImpl(T1 mP1, T2 mP2, TArgs... mArgs)
        {
            ctorImpl(mP1);
            ctorImpl(mP2, mArgs...);
        }
        ...
        inline bool matchesImpl(const RulePart<TL>& mPart) const
        {
            return mPart == ruleParts.at(TIdx);
        }
        template <std::size_t TIdx, typename T1, typename T2, typename... TArgs>
        inline bool matchesImpl(T1 mP1, T2 mP2, TArgs... mArgs) const
        {
            return !matchesImpl<TIdx>(mP1) ? false : matchesImpl<TIdx + 1>(
                                                         mP2, mArgs...);
        }

    public:
        template <typename... TArgs>
        inline RuleKey(TArgs... mArgs)
        {
            ctorImpl(mArgs...);
        }

        template <typename... TArgs>
        inline bool matches(TArgs... mArgs)
        {
            return sizeof...(TArgs) != ruleParts.size()
                       ? false
                       : matchesImpl<0>(mArgs...);
        }
    ...
    public:
        ...
        template <typename T>
        T& get(std::size_t mIdx) noexcept
        {
    ...
    public:
        inline Rule(RuleKey<TL> mKey, const RuleFunc& mFunc)
            : ruleKey{std::move(mKey)}, func{mFunc}
        {
        }

        inline std::size_t getSize() const noexcept
        {
            return ruleKey.getSize();
        }
        inline const RulePart<TL>& getPartAt(std::size_t mIdx) const noexcept
        {
            return ruleKey.getPartAt(mIdx);
        }
    ...
    public:
        template <typename... TArgs>
        inline Rule<TL>& createRule(TArgs&&... mArgs)
        {
            return ssvu::getEmplaceUPtr<Rule<TL>>(
                rules, std::forward<TArgs>(mArgs)...);
        }

        inline const decltype(rules)& getRules() const noexcept
        {
            return rules;
        }
    };
    ...
    public:
        ...
        inline bool parseStackMatchesRule(
            std::size_t mStartAt, const Rule<TL>& mRule)
        {
        ...
        inline void reduceRecursively()
        {
            for(const auto& r : ruleSet.getRules())
            {
                ...
                for(auto i(0u); i < parseStack.size(); ++i)
                {
                    ...
                    // Remove base nodes from parse stack (they will be
                    // substitued with the reduction result)
                    std::vector<ASTNodePtr<TL>> usedNodes;
                    for(auto i(0u); i < r->getSize(); ++i)
                    {
                        usedNodes.emplace(
                            std::begin(usedNodes), parseStack.back());
                        parseStack.pop_back();
                    }

                    // Create reduction result node
                    auto& reductionResult(
                        ssvu::getEmplaceUPtr<ASTNode<TL>>(nodeManager));

                    NodeCtx<TL> nodeCtx{usedNodes};
                    reductionResult.setImpl(std::move(r->getFunc()(nodeCtx)));

                    // Remove base nodes from node stack (they will become
                    // children of the redution result node)
                    ...
                    // Pop N nodes from node stack into removedNodes, then try
                    // reducing further
                    reduceRecursively();
                    return;
                }
...
namespace Lang
{
    ...
    struct ASTNumber : public ASTExpr
    {
        ...
        inline int eval() override { return value; }

        inline void toBytecode() override
        {
            ssvu::lo() << "push Number" << std::endl;
        }
    };
    ...
    struct ASTParenthesizedExpr : public ASTExpr
    {
        ...
        inline ASTParenthesizedExpr(ASTExpr& mInnerExpr)
            : innerExpr{&mInnerExpr}
        {
        }
    ...
    struct OpAddition
    {
        inline static T get(const T& mA, const T& mB) noexcept
        {
            return mA + mB;
        }
    };
    ...
    struct ASTBinaryOp : public ASTExpr
    {
        ...
        inline ASTBinaryOp(ASTExpr& mLhs, ASTExpr& mRhs)
            : lhs{&mLhs}, rhs{&mRhs}
        {
        }
        ...
        inline void toBytecode() override
        {
            lhs->toBytecode();
            rhs->toBytecode();
            ssvu::lo() << "pop Sum" << std::endl;
        }
    };
}

template <bool TFmt, typename TN>
inline void printNode(std::ostream& mStream, TN& mValue, int mDepth)
{
    ...
    // ssvu::Internal::callStringifyImpl<TFmt>(mStream, mValue.getTypeId() < -1
    // ? mValue.template getImplAs<Lang::ASTExpr>().getName()
    //	: ssvu::toStr(int(mValue.template
    // getImplAs<Eng::ASTTokenNodeImpl<Lang::Spec>>().getToken().getType())));
...
int main()
{
    ...
    ruleSet.createRule(
        RuleKey<Spec>{Tkn::POpen, getASTTypeId<ASTExpr>(), Tkn::PClose},
        [](NodeCtx<Spec>& mCtx)
        {
    ...
    ruleSet.createRule(RuleKey<Spec>{getASTTypeId<ASTNumber>()},
        [](NodeCtx<Spec>& mCtx)
        {
    ...
    ruleSet.createRule(
        RuleKey<Spec>{getASTTypeId<ASTBinaryOp<OpAddition<int>>>()},
        [](NodeCtx<Spec>& mCtx)
        {
            ssvu::lo("EXPR") << "" << std::endl;
            return createASTImpl<Spec, ASTExpr>();
        });
    ruleSet.createRule(RuleKey<Spec>{getASTTypeId<ASTExpr>(), Tkn::OpPlus,
                           getASTTypeId<ASTExpr>()},
        [](NodeCtx<Spec>& mCtx)
        {
            ssvu::lo("PLUSEXPR") << "" << std::endl;
            return createASTImpl<Spec, ASTBinaryOp<OpAddition<int>>>(
                mCtx.get<ASTExpr>(0), mCtx.get<ASTExpr>(2));
        });

    // figure a way of not having to specify all inherited class of a certain
    // class for expansions

    std::vector<Token<Spec>> src{{Tkn::POpen}, {Tkn::Number}, {Tkn::OpPlus},
        {Tkn::Number}, {Tkn::PClose}, {Tkn::OpPlus}, {Tkn::POpen},
        {Tkn::Number}, {Tkn::OpPlus}, {Tkn::Number}, {Tkn::PClose}};
