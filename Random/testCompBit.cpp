...
template <ComponentBitType TB>
struct ComponentBit : std::integral_constant<ComponentBitType, TB>
{
};
...
{
    static ComponentBitType result{0};
    return result++;
}
template <typename T>
inline auto getSId() noexcept
{
    static auto result(getLast());
    return result;
}
...
struct TypeId
{
    static ComponentBitType id;
};
template <typename T>
ComponentBitType TypeId<T>::id{getLast()};

template <typename T>
inline auto getSIdImp() noexcept
{
    return TypeId<T>::id;
}
