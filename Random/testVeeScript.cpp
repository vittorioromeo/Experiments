...
namespace ssvvs
{
    ...
    SSVU_FATENUM_DEFS(EnumMgr, TokenMetaType, int, Value, Keyword, Operator,
        Serparator, Special);
    ...
    SSVU_FATENUM_DEFS(EnumMgr, TokenType, int,
        ...
        // Keyword tokens
        If, While, True, False, Null, Var,

        // Operator tokens
        Plus, Minus, Star, Slash, Percent, PrRoundL, PrRoundR, Equal,

        // Separator tokens
        Colon, Semicolon);

    SSVU_FATENUM_DEFS(EnumMgr, ValueType, int, Null, Int, Float);

    SSVU_FATENUM_DEFS(
        EnumMgr, OperatorType, int, Plus, Minus, Star, Slash, Percent);
    ...
    struct Token
    {
        ...
        inline Token(
            TokenMetaType mMetaType, TokenType mType, const TokenData& mData)
            : metaType{mMetaType}, type{mType}, data(mData)
        {
        }
    };
    ...
    public:
        inline ASTNode& createASTRoot() { return create<ASTNode>(); }

        template <typename T, typename... TArgs>
        inline ASTNode& create(TArgs&&... mArgs)
        {
    ...
    struct Value
    {
        ...
        template <typename T>
        inline Value(ValueType mType, T mValue)
            : type(mType)
        {
            switch(type)
            {
        ...
        {
            if(type == ValueType::Null || mRhs.type == ValueType::Null)
                return {ValueType::Null};
            if(type == ValueType::Float || mRhs.type == ValueType::Float)
                return {ValueType::Float, getAsFloat() + mRhs.getAsFloat()};
        ...
        inline Value operator%(const Value& mRhs) const noexcept
        {
            ...
            if(type == ValueType::Float || mRhs.type == ValueType::Float)
                return {ValueType::Float,
                    std::fmod(getAsFloat(), mRhs.getAsFloat())};
            return {ValueType::Int, getAsInt() % mRhs.getAsInt()};
        }
    };
    ...
    struct ASTEOp : ASTExpr
    {
        ...
        inline ASTEOp(OperatorType mOperatorType, ASTExpr* mLhs, ASTExpr* mRhs)
            : op{mOperatorType}, lhs{mLhs}, rhs{mRhs}
        {
        }
...
int main()
{
    ...
    auto& nv1(context.create<ASTEValue>(Value{ValueType::Float, 10.f})
                  .getAs<ASTExpr>());
    auto& nv2(context.create<ASTEValue>(Value{ValueType::Float, 3.f})
                  .getAs<ASTExpr>());
