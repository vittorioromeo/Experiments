...
{
    template <char TC>
    using CTChar = IntegralConstant<char, TC>;
    template <char... TCs>
    using ListChar = MPL::ListIC<char, TCs...>;

    namespace Impl
    {
        template <SizeT TN>
        inline constexpr char getCTStrC(const char(&mS)[TN], SizeT mI) noexcept
        {
            return mI >= TN ? '\0' : mS[mI];
        }

        template <SizeT TN>
        inline constexpr char getCTStrSize(const char(&)[TN]) noexcept
        {
            return TN;
        }

        template <typename T>
        struct IsCTNonZero : IntegralConstant<bool, !isSame<T, CTChar<'\0'>>()>
        {
        };
    }

    template <typename TL>
    using TrimExtraZeros = typename TL::template Filter<Impl::IsCTNonZero>;

    namespace CTLiteral
    {
        template <typename T, T... TChars>
        inline constexpr TrimExtraZeros<ListChar<TChars...>> operator""_cts()
        {
            return {};
        }
    }
    /*
        ...
        {
            inline static constexpr char* get() noexcept { return
       std::array<char, sizeof...(TChars)>{{ TChars... }}.data(); }
        };

        template<typename TL, int TN> inline constexpr char nthChar() noexcept {
       return ToCharArray<TL>::get()[TN]; }
    */
}
...
#define SSVCTS_CHARS_IMPL(mIdx, mData) \
    SSVPP_COMMA_IF(mIdx)::ssvu::Impl::getCTStrC(mData, mIdx)
#define SSVCTS_CHARS_WLEN(mLen, mStr) \
    SSVPP_IMPL_REPEAT_INC(mLen, SSVCTS_CHARS_IMPL, mStr)
#define SSVCTS_CHARS(mStr) SSVCTS_CHARS_WLEN(CTSTR_MAX_SIZE, mStr)
// #define SSVCTS_WLEN(mLen, mStr) ::ssvu::CTString<SSVCTS_CHARS_WLEN(mLen,
// mStr)>
#define SSVU_CHARLIST(mStr) \
    ::ssvu::TrimExtraZeros<::ssvu::ListChar<SSVCTS_CHARS(mStr)>>
...
#define SSVCTS_REPLACE_ALL(mSrc, mNew) \
    ReplaceAllOfSeq<SSVCTS(mSrc), SSVCTS(mNew)>
...
int main()
{
    ...
    SSVU_ASSERT_STATIC_NM(isSame<ListChar<'h', 'e', 'y'>::PushBack<CTChar<'u'>>,
        ListChar<'h', 'e', 'y', 'u'>>());
    ...
    SSVU_ASSERT_STATIC_NM(
        isSame<ListChar<'h', 'e', 'y'>::Append<ListChar<'b', 'r', 'o'>>,
            ListChar<'h', 'e', 'y', 'b', 'r', 'o'>>());
    SSVU_ASSERT_STATIC_NM(
        isSame<SSVCTS("hey")::SSVCTS_ADD("bro"), SSVCTS("heybro")>());
    ...
    SSVU_ASSERT_STATIC_NM(
        isSame<SSVCTS("hey")::SSVCTS_SUBSTR(0, 2), SSVCTS("he")>());

    SSVU_ASSERT_STATIC_NM(
        isSame<SSVCTS("hey banana")::SSVCTS_IDXOFSEQ("banana"), ListInt<4>>());

    // Too slow
    SSVU_ASSERT_STATIC_NM(
        isSame<SSVCTS("hey bana bana")::SSVCTS_IDXOFSEQ("bana"),
            ListInt<4, 9>>());

    // If the string is any longer, it kills the compiler. Problem is in MPL
    // ReplaceAllOfSeq
    // SSVU_ASSERT_STATIC_NM(isSame<SSVCTS("hey
    // banana")::SSVCTS_REPLACE_ALL("banana", "apple"), SSVCTS("hey apple")>());
    // SSVU_ASSERT_STATIC_NM(isSame<SSVCTS("hey banana hey
    // banana")::SSVCTS_REPLACE_ALL("banana", "apple"), SSVCTS("hey apple hey
    // apple")>());

    /*SSVU_ASSERT_STATIC_NM(ssvu::isSame<ssvu::ListChar<'h', 'e', 'e', 'y',
'\0', '\0', '\0'>::TrimExtraZeros, ssvu::ListChar<'h', 'e', 'e', 'y', '\0'>>());
