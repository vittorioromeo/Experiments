...
namespace Batch
{
    namespace Impl
    {
        ...
        struct TextureData
        {
            ...
            {
                set(ssvu::toFloat(mTexture.getSize().x),
                    ssvu::toFloat(mTexture.getSize().y));
            }
        };
    }
    ...
    public:
        ...
        template <typename T>
        inline void setPosition(T&& mX) noexcept
        {
            position = FWD(mX);
        }
        template <typename T>
        inline void setOrigin(T&& mX) noexcept
        {
            origin = FWD(mX);
        }
        template <typename T>
        inline void setScale(T&& mX) noexcept
        {
            scale = FWD(mX);
        }
        ...
        inline const auto& getHalfTextureSize() const noexcept
        {
            return textureData.halfSize;
        }
    ...
    private:
        inline auto getRotatedPoint(float mX, float mY, float mSSin,
            float mSCos, const sf::Vector2f& mTOrigin) const noexcept
        {
        ...
        template <typename T>
        inline void emplaceVertices(T& mV) const
        {
            ...
            auto erv([this, &d, &tOrigin, &sSin, &sCos](
        ...
        inline void drawOn(sf::RenderTarget& mX) noexcept
        {
            ...
            while(curr != end)
            {
                ...
                mX.draw(vs.getDataPtr(), cv - vs.getDataPtr(),
                    sf::PrimitiveType::Quads, sf::RenderStates{tx});
            }

            zvs.clear();
        }

        inline void enqueue(const BatchSprite& mX) { mX.emplaceVertices(zvs); }
    ...
    /*inline void Impl::Layer::drawOn(Manager& mManager, sf::RenderTarget& mX)
    noexcept
    {
...
int main()
{
    ...
    for(auto i(0u); i < 150000; ++i)
    {
        ...
        l.spr.setPosition(
            sf::Vector2f{ssvu::getRndR(0.f, 800.f), ssvu::getRndR(0.f, 600.f)});
