...
template<typename T, typename... TArgs> class RangeZip 
{
	
};

template<typename T> inline constexpr auto getReverseRange(const T& mContainer) noexcept 
	-> Range<decltype(mContainer.rbegin())> 
{
	return {mContainer.rbegin(), mContainer.rend()};
}

template<typename T> inline constexpr auto getHalfRange(const T& mContainer) noexcept 
	-> Range<decltype(mContainer.begin())> 
{
	return {mContainer.begin(), mContainer.begin() + (mContainer.end() - mContainer.begin()) / 2};
}
