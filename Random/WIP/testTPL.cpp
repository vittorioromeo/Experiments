...
template<typename T> class IsCopyAssignable
{
	private:
...
//                 ^ these types have to be well-formed for void_t to instantiate(?)
//				   (SFINAE-friendly: it either gives `void` or NO ANSWER)

template<typename, typename = void> struct HasHelloTypeMember : std::false_type { };
template<typename T> struct HasHelloTypeMember<T, void_t<typename T::hello>> : std::true_type { };
//																  ^
//		If T::hello does not exist, this is ill-formed and void_t SFINAE kicks in 

template<typename T> using VTCopyAssignmentHelper 
	= decltype(std::declval<T&>() = std::declval<const T&>);
template<typename T, typename = void> struct IsVTCopyAssignable : std::false_type { };
template<typename T> struct IsVTCopyAssignable<T, void_t<VTCopyAssignmentHelper<T>>>
...
struct TestHello1 { int hello; };
struct TestHello2 { void hello() { } };
struct TestHello3 { using hello = int; };
