...
namespace test
{
    ...
    public:
        inline std::size_t insertAndGetIdx(T mValue)
        {
            auto lb(std::lower_bound(
                std::begin(items), std::end(items), mValue, comparer));
        ...
        inline const decltype(items)& getItems() const noexcept
        {
            return items;
        }
    };
    ...
    template <typename TKey, typename TValue,
        typename TComparer = std::less<TKey>>
    class DODMap
    {
    private:
    ...
    public:
        inline DODMap()
            : keys{new TKey[defaultCapacity]},
              values{new TValue[defaultCapacity]}, size{0u},
...
int main()
{
        ...
        {
            {
                ...
                for(auto j(0u); j < jj; ++j)
                    map.emplace_back(
                        std::make_pair<std::size_t, BigObj>(j, {}));
            }

            {
                SSVU_BENCHMARK_LOG_SCOPE_EXIT("lookup");
                ssvu::sort(map, [](const auto& mA, const auto& mB)
                    {
                        return mA.first < mB.first;
                    });
                for(auto j(0u); j < jj; ++j)
                    ssvu::lowerBound(map, j,
    ...
    {
        SSVU_BENCHMARK_LOG_SCOPE_EXIT("ssvu::VecMap");
        ssvu::VecMap<std::size_t, BigObj> map;
