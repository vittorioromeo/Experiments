...
<
	typename T, 
	CBPGrow TPGrow = CBPGrow::Automatic, 
...
class CircularBuffer
{
	private:
		...
		inline auto getIdx(SizeT mX) const noexcept
		{			
			return mX % capacity;
		}

	public:
		inline CircularBuffer(SizeT mCapacity = 10)
		{			
			growTo(mCapacity);
		}

		template<typename... TArgs> inline auto& emplace(TArgs&&... mArgs)
		{
			growIfNeeded();
		...
		inline const auto& get(SizeT mI) const noexcept { return data[getIdx(iHead + mI)]; }
...
template<typename T> class TSCircularBuffer
{
	public:
	...
	public:
		template<typename... TArgs> inline auto& enqueue(TArgs&&... mArgs)
		{
...
class ManagedSocket
{
	...
	private:
		...
		template<typename TP> inline auto& tsEnqueueIn(TP&& mP) { return tscbIn.enqueue(FWD(mP)); }
		inline auto& tsDequeueIn() { return tscbIn.dequeue(); }

		template<typename TP> inline auto& tsEnqueueOut(TP&& mP) { return tscbOut.enqueue(FWD(mP)); }
		inline auto& tsDequeueOut() { return tscbOut.dequeue(); }

		template<typename TF> inline void whileBusy(TF mFn)
		{
			while(busy)
			{
				mFn();
