...
void mkCat(SizeT i)
{
    output << "#define SSVPP_IMPL_TPL_CAT_" << i
           ...
           << "(SSVPP_TPL_MAKE(SSVPP_TPL_EXPLODE(m0), SSVPP_TPL_EXPLODE(m1)), "
              "__VA_ARGS__)\n";
}
...
void genTuple()
{
    ...
    output << "#define SSVPP_IMPL_TPL_FILL_" << tplCount
           << "(m0)	SSVPP_TPL_EXPLODE(m0)\n";
    output << "\n";

    // tpl fill reverse
    for(auto i(0u); i < tplCount; ++i) mkFillReverse(i);
    output << "#define SSVPP_IMPL_TPL_FILL_REVERSE_" << tplCount
           << "(m0)	SSVPP_TPL_EXPLODE(m0)\n";
    output << "\n";
    ...
    output << "#define SSVPP_IMPL_TPL_CAT_2(m0, m1)		"
              "SSVPP_TPL_MAKE(SSVPP_TPL_EXPLODE(m0), SSVPP_TPL_EXPLODE(m1))\n";
...
void genArithmetic()
{
    ...
    for(auto i(3u); i < arithCount; ++i)
        output << "#define SSVPP_IMPL_DECREMENT_" << i << " " << i - 1 << "\n";
    output << "\n";

    // increment
    for(auto i(0u); i < arithCount - 1; ++i)
        output << "#define SSVPP_IMPL_INCREMENT_" << i << " " << i + 1 << "\n";
    output << "#define SSVPP_IMPL_INCREMENT_" << arithCount - 1 << " "
           << arithCount << "\n";
    ...
    for(auto i(1u); i < arithCount; ++i)
        output << "#define SSVPP_IMPL_BOOL_" << i << " 1\n";
    output << "\n";
}
...
void genArgCount()
{
    ...
    output << "#define SSVPP_IMPL_CAT_2(m0, m1)		"
              "SSVPP_IMPL_CAT_1(SSVPP_TKNCAT_2(m0, m1))\n";
    for(auto i(3u); i < argCount; ++i)
        output << "#define SSVPP_IMPL_CAT_" << i
               << "(m0, m1, ...)	SSVPP_IMPL_CAT_" << i - 1
               << "(SSVPP_TKNCAT_2(m0, m1), __VA_ARGS__)\n";
    output << "\n";
}
...
{
    output
        << "#define SSVPP_IMPL_FOREACH_1(mAction, mData, mA0)				"
           "mAction(0, mData, mA0)\n";
    output
        << "#define SSVPP_IMPL_FOREACH_2(mAction, mData, mA0, mA1)			"
           "mAction(1, mData, mA0) SSVPP_IMPL_FOREACH_1(mAction, mData, mA1)\n";

    for(auto i(2u); i < foreachCount; ++i)
    {
        output << "#define SSVPP_IMPL_FOREACH_" << i + 1;
        output << "(mAction, mData, mA0, mA1, ...)		";
        output << "mAction(" << i << ", mData, mA0) SSVPP_IMPL_FOREACH_" << i
...
void genRepeat()
{
    ...
    for(auto i(0u); i < repeatCount; ++i)
        output << "#define SSVPP_IMPL_REPEAT_DEC_" << i + 1
               << "(mAction, mData) mAction(" << i
               << ", mData) SSVPP_IMPL_REPEAT_DEC_" << i
               << "(mAction, mData)\n";
    ...
    for(auto i(0u); i < repeatCount; ++i)
        output << "#define SSVPP_IMPL_REPEAT_INC_" << i + 1
               << "(mAction, mData, mLast) mAction(mLast, mData) "
                  "SSVPP_IMPL_REPEAT_INC_"
               << i << "(mAction, mData, SSVPP_INCREMENT(mLast))\n";
}
