...
#define TW_TYPEOF(mX) TypeOf<decltype(mX)>
        ...
        inline void forTypes(TF&&) noexcept
        {
        }
    ...
    namespace Impl
    {
        ...
        private:
            ...
            template <typename T>
            inline static constexpr auto getTypeID() noexcept
            {
        ...
        public:
            inline ~TaggedVariant() { deinit(); }

            template <typename T>
            inline auto is() const noexcept
            {
                return typeID == getTypeID<T>();
            }

            inline void setNull() { deinit(); }

            inline auto isNull() const noexcept { return typeID == nullTypeID; }
    ...
    template <typename... Ts>
    using TaggedVariant = Impl::TaggedVariant<Impl::VariantStorageUnion, Ts...>;
}
