...
namespace ssvu
{
    ...
    private:
        ...
        inline void createImpl(TArgs&&... mArgs) // noexcept(...)
        {
            ...
            // TODO: instead of passing margs, pass a single tuple argument that
            // contains N tuples containing the
    ...
    public:
        ...
        template <typename T>
        inline auto forSinglePtr() noexcept
        {
            return makeRange(beginSingle(), endSingle());
        }
        template <typename T>
        inline auto forNextSinglePtr() noexcept
        {
            return makeRange(beginSingle(), endNextSingle());
        }

        template <typename T>
        inline auto forSinglePtr() const noexcept
        {
            return makeRange(beginSingle(), endSingle());
        }
        template <typename T>
        inline auto forNextSinglePtr() const noexcept
        {
            return makeRange(beginSingle(), endNextSingle());
        }

        template <typename T>
        inline auto forSingleIdx() noexcept
        {
            return makeRange(beginSingleIdx(), endSingleIdx());
        }
        template <typename T>
        inline auto forNextSingleIdx() noexcept
        {
            return makeRange(beginSingleIdx(), endNextSingleIdx());
        }

        template <typename T>
        inline auto forSingleIdx() const noexcept
        {
            return makeRange(beginSingleIdx(), endSingleIdx());
        }
        template <typename T>
        inline auto forNextSingleIdx() const noexcept
        {
            return makeRange(beginSingleIdx(), endNextSingleIdx());
        }
    };
