...
namespace Exec
{
	class RunTime
	{
		private:
			...
			template<typename T> RunTime(T&& mObj) : obj(std::forward<T>(mObj))
			{
		...
		public:
			...
			template<typename T, typename... TArgs> void reset(TArgs&&... mArgs)
			{
				obj = ssvu::makeUPtr<T>(std::forward<TArgs>(mArgs)...);
			}
	};

	template<typename T> class CompileTime 
	{
		private:
			T obj;

		public:
			template<typename... TArgs> CompileTime(TArgs&&... mArgs)
...
template<typename TExec> class Manager
{
	private:
		TExec exec;

	public:
		template<typename... TArgs> Manager(TArgs&&... mArgs)
