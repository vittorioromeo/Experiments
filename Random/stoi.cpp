...
inline auto& getSS() noexcept { thread_local std::ostringstream oss; return oss; }
template<typename T> inline auto ss(T x) { getSS().str(""); getSS() << x; return getSS().str(); }

template<typename T> inline auto getri() noexcept { return ssvu::getRndI<T, T>(ssvu::NumLimits<T>::min(), ssvu::NumLimits<T>::max()); }

template<typename T> inline void check(T v, const std::string& mX)
{
...
namespace ssvu
{
	namespace Impl
	{
		namespace NumToStr
		{
			template<SizeT TS> inline auto& getConvBuffer() noexcept
			{
				thread_local std::array<char, TS> result;
				return result;
			}

			template<typename T, class TEnable = void> struct Conv;
			...
			template<typename T> struct Conv<T, EnableIf<!isUnsigned<T>()>>
			{
			...
			template<typename T> inline auto toStr(const T& mX) noexcept
			{
...
template<typename T> inline std::string veenaivei(T x) noexcept
{
	return ssvu::Impl::NumToStr::toStr(x);
}


template<typename T> inline void runTest() noexcept
{	
	{
