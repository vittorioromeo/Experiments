...
#define SSVU_CTR_INC(mTag)                                                \
    CtrConstantIndex<SSVU_CTR_GET(mTag) + 1> constexpr getCtrCrumb(mTag,  \
        CtrConstantIndex<(SSVU_CTR_GET(mTag) + 1) & ~SSVU_CTR_GET(mTag)>, \
        CtrConstantIndex<(SSVU_CTR_GET(mTag) + 1) & SSVU_CTR_GET(mTag)>)  \
    {                                                                     \
        return {};                                                        \
    }
...
#define REGISTERED_CLASS(mTag, mKey, mName)                        \
    mKey mName& register_type(mTag, decltype(SSVU_CTR_GET(mTag))); \
    SSVU_CTR_INC(mTag) mKey mName

template <std::size_t TS>
struct CtrConstantIndex : std::integral_constant<std::size_t, TS>
{
};

template <typename TID, std::size_t TRank, std::size_t TAcc>
constexpr CtrConstantIndex<TAcc> getCtrCrumb(
    TID, CtrConstantIndex<TRank>, CtrConstantIndex<TAcc>)
{
    return {};
}
