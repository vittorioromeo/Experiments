...
namespace ssvu
{
	namespace Impl
	{
		template<typename T, template<typename...> class TUV> class MaybeImpl
		{
			protected:
				TUV<T> uv;
			...
			public:
				template<typename... TArgs> inline void init(TArgs&&... mArgs)
					noexcept(noexcept(uv.template init<T>(SSVU_FWD(mArgs)...)))
				{
				...
				inline const T get() && noexcept { return std::move(uv.template get<T>()); }
		};
	}

	template<typename T> struct Maybe : public Impl::MaybeImpl<T, UnionVariant>
	{
	...
	template<typename T> using MaybePOD = Impl::MaybeImpl<T, UnionVariantPOD>;
}
