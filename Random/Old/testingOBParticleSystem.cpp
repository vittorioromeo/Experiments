...
namespace ob
{
    namespace t
    {
        ...
        private:
            ...
            inline Handle(HManager<T>& mManager, Idx mMarkIdx,
                Ctr mCtr) noexcept : manager(&mManager),
                                     markIdx{mMarkIdx},
                                     ctr{mCtr}
            {
            }
        ...
        public:
            inline AtomType& getAtom() noexcept
            {
                return getAtomImpl<AtomType&>();
            }
        ...
        private:
            ...
            inline void growCapacityBy(std::size_t mAmount)
            {
                ...
                for(; i < newCapacity; ++i)
                    atoms[i].markIdx = marks[i].atomIdx = i;
            }
            ...
            inline void growIfNeeded()
            {
                ...
                if(getCapacity() <= sizeNext)
                    growCapacityTo(
                        (getCapacity() + growAmount) * growMultiplier);
            }
        ...
        public:
            ...
            template <typename... TArgs>
            inline AtomType& createAtom(TArgs&&... mArgs)
            {
            ...
            {
                return createHandleFromAtom(
                    createAtom(std::forward<TArgs>(mArgs)...));
            }
            ...
            inline void refresh()
            {
                ...
                for(int iD{iDead}; iD < intSizeNext; ++iD)
                {
                    ...
                    for(int iA{iDead + 1}; true; ++iA)
                    {
                        ...
                        // Found an alive atom after dead `iD` atom - swap and
                        // update mark
            ...
            finishRefresh:
                ...
                // [iAlive + 1, intSizeNext) contains only dead atoms, clean
                // them up
            ...
            inline T& getDataAt(Idx mIdx) noexcept
            {
    ...
    public:
        ...
        {
            if(particles.getSizeNext() <= OBConfig::getParticleMax())
                particles.createAtom(std::forward<TArgs>(mArgs)...);
        }
        inline void update(FT mFT)
        {
            // Remove excess particles
            // if(particles.size() > OBConfig::getParticleMax())
            // particles.erase(std::begin(particles) +
            // OBConfig::getParticleMax(), std::end(particles));

            // ssvu::eraseRemoveIf(particles, [](const OBParticle& mParticle){
            // return mParticle.life <= 0; });
            particles.refresh();
            particles.forEachAtom([](decltype(particles)::AtomType& mA)
                {
                    if(mA.getData().life <= 0) mA.setDead();
                });
