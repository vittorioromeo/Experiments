...
#define IMPL_IMPL_FORNARGS_EXECN_BODY() FWD(mFn)(std::get<N + Cs>(FWD(mXs))...)
...
#define IMPL_IMPL_FORNARGS_EXEC_BODY()                                   \
    (void) Swallow                                                       \
    {                                                                    \
        (execN<(Bs * sizeof...(Cs))>(FWD(mFn), FWD(mXs)), true)..., true \
    }
...
#define IMPL_FORNARGS_BODY()                                            \
    Impl::ForNArgsHlpr<MkIdxSeq<sizeof...(Ts) / N>, MkIdxSeq<N>>::exec( \
        FWD(mFn), fwdAsTpl(FWD(mXs)...))

    template <std::size_t N, typename TF, typename... Ts>
    inline constexpr void forNArgs(TF&& mFn, Ts&&... mXs) noexcept(
        noexcept(IMPL_FORNARGS_BODY()))
    {
        SSVU_ASSERT_STATIC(sizeof...(Ts) % N == 0,
...
int main()
{
    ...
    forNArgs<4>(
        [&x](auto&& mA1, auto&& mA2, auto&& mA3, auto&& mA4)
        {
            x += mA1 * mA2 + mA3 + mA4;
        },
        1, 1, 4, 6);
