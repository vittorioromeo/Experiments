...
int main()
{
    ...
    {
        {
            Benchmark::start(); { for(int i = 0; i < repeats; ++i) rawFunc(tempVar, i); } lo("raw func") << Benchmark::getEndString() << endl;
        }
        {
            function<int(int&, int)> t1 = &rawFunc;
            Benchmark::start(); { for(int i = 0; i < repeats; ++i) t1(tempVar, i); } lo("raw func std::func") << Benchmark::getEndString() << endl;
        }
        {
            FastFunc<int(int&, int)> t1 = &rawFunc;
            Benchmark::start(); { for(int i = 0; i < repeats; ++i) t1(tempVar, i); } lo("raw func don_delegate") << Benchmark::getEndString() << endl;
        }
      lo() << endl;

        {
            Benchmark::start(); { for(int i = 0; i < repeats; ++i) testStr.rawMemFunc(tempVar); } lo("raw memfunc") << Benchmark::getEndString() << endl;
        }
        {
            function<void()> t1 = [&]{ testStr.rawMemFunc(tempVar); };
            Benchmark::start(); { for(int i = 0; i < repeats; ++i) t1(); } lo("raw memfunc  std::func") << Benchmark::getEndString() << endl;
        }
        {
            FastFunc<void(int&)> t1(&testStr, &TestStruct::rawMemFunc);
            Benchmark::start(); { for(int i = 0; i < repeats; ++i) t1(tempVar); } lo("raw memfunc  don_delegate") << Benchmark::getEndString() << endl;
        }
      lo() << endl;

        {
            auto t1 = [](int i){ tempGlobalState = !tempGlobalState; return tempGlobalVar + i + 1; };
            Benchmark::start(); { for(int i = 0; i < repeats; ++i) t1(i); } lo("trivial auto") << Benchmark::getEndString() << endl;
        }
        {
            function<int(int)> t1 = [](int i){ tempGlobalState = !tempGlobalState; return tempGlobalVar + i + 1; };
            Benchmark::start(); { for(int i = 0; i < repeats; ++i) t1(i); } lo("trivial std::func") << Benchmark::getEndString() << endl;
        }
        {
            FastFunc<int(int)> t1 = [](int i){ tempGlobalState = !tempGlobalState; return tempGlobalVar + i + 1; };
            Benchmark::start(); { for(int i = 0; i < repeats; ++i) t1(i); } lo("trivial don_delegate") << Benchmark::getEndString() << endl;
        }
        lo() << endl;

        {
            ...
            Benchmark::start(); { for(int i = 0; i < repeats; ++i) t1(i); } lo("capture std::func") << Benchmark::getEndString() << endl;
        }
        {
            FastFunc<int(int&, int)> t2 = rawFunc;
            FastFunc<void(int)> t1 = [&tempVar, &t2](int i){ tempVar = t2(tempVar, i); };
            Benchmark::start(); { for(int i = 0; i < repeats; ++i) t1(i); } lo("capture don_delegate") << Benchmark::getEndString() << endl;
        }
        lo() << endl;
