...
namespace ssvu
{
    namespace Internal
    {
        ...
        inline TOut horrible_cast(TIn mIn) noexcept
        {
            ...
            static_assert(
                sizeof(TIn) == sizeof(u) && sizeof(TIn) == sizeof(TOut),
        ...
        struct SimplifyMemFunc
        {
            ...
            inline static AnyPtrThis convert(
                const TThis*, TFunc, AnyPtrFunc&) noexcept
            {
        ...
        struct SimplifyMemFunc<SingleMemFuncPtrSize>
        {
            template <class TThis, class TFunc>
            inline static AnyPtrThis convert(
                const TThis* mThis, TFunc mFunc, AnyPtrFunc& mFuncOut) noexcept
            {
        ...
        public:
            ...
            {
                ptrThis = SimplifyMemFunc<sizeof(mFunc)>::convert(
                    mThis, mFunc, ptrFunction);
            }
            template <class TThis, class TInvoker>
            inline void bind(
                TThis* mThis, TInvoker mInvoker, PtrStaticFuncT mFunc) noexcept
            {
                if(mFunc == nullptr)
                    ptrFunction = nullptr;
                else
            ...
            inline bool operator==(PtrStaticFuncT mPtr) const noexcept
            {
                return mPtr == nullptr
                           ? *this == nullptr
                           : mPtr == reinterpret_cast<PtrStaticFuncT>(
                                         getStaticFunc());
            }
            inline bool operator!=(std::nullptr_t) const noexcept
            {
        ...
        private:
            ...
            {
                return (*(closure.getStaticFunc()))(mArgs...);
            }
...
int rawFunc(int& xx, int i)
{
    xx += i;
    tempGlobalState = !tempGlobalState;
    return i + 1;
}
struct TestStruct
{
    void rawMemFunc(int& xx) const
    {
        tempGlobalState = !tempGlobalState;
        xx += 10;
    }
...
int main()
{
    ...
    if(testEqual == nullptr)
        testEqual = []
        {
            lo() << "hello from testequal (trivial)" << endl;
        };
    testEqual();
    if(testEqual != nullptr)
        testEqual = [&tempStr]
    ...
    {
        {
            Benchmark::start();
            {
                for(int i = 0; i < repeats; ++i) rawFunc(tempVar, i);
            }
            lo("raw func") << Benchmark::getEndString() << endl;
        }
        {
            function<int(int&, int)> t1 = &rawFunc;
            Benchmark::start();
            {
                for(int i = 0; i < repeats; ++i) t1(tempVar, i);
            }
            lo("raw func std::func") << Benchmark::getEndString() << endl;
        }
        {
            ...
            lo("raw func don_delegate") << Benchmark::getEndString() << endl;
        }
        lo() << endl;

        {
            Benchmark::start();
            {
                for(int i = 0; i < repeats; ++i) testStr.rawMemFunc(tempVar);
            }
            lo("raw memfunc") << Benchmark::getEndString() << endl;
        }
        {
            function<void()> t1 = [&]
            {
                testStr.rawMemFunc(tempVar);
            };
            Benchmark::start();
            {
                for(int i = 0; i < repeats; ++i) t1();
            }
            lo("raw memfunc  std::func") << Benchmark::getEndString() << endl;
        }
        {
            FastFunc<void(int&)> t1(&testStr, &TestStruct::rawMemFunc);
            Benchmark::start();
            {
                for(int i = 0; i < repeats; ++i) t1(tempVar);
            }
            lo("raw memfunc  don_delegate") << Benchmark::getEndString()
                                            << endl;
        }
        lo() << endl;

        {
            ...
            {
                tempGlobalState = !tempGlobalState;
                return tempGlobalVar + i + 1;
            };
            Benchmark::start();
            {
                for(int i = 0; i < repeats; ++i) t1(i);
            }
            lo("trivial auto") << Benchmark::getEndString() << endl;
        }
        {
            function<int(int)> t1 = [](int i)
            {
                tempGlobalState = !tempGlobalState;
                return tempGlobalVar + i + 1;
            };
            ...
            lo("trivial std::func") << Benchmark::getEndString() << endl;
        }
        {
            FastFunc<int(int)> t1 = [](int i)
            {
                tempGlobalState = !tempGlobalState;
                return tempGlobalVar + i + 1;
            };
            ...
            lo("trivial don_delegate") << Benchmark::getEndString() << endl;
        }
        lo() << endl;

        {
            function<int(int&, int)> t2 = rawFunc;
            function<void(int)> t1 = [&tempVar, &t2](int i)
            {
                tempVar = t2(tempVar, i);
            };
            Benchmark::start();
            {
                for(int i = 0; i < repeats; ++i) t1(i);
            }
            lo("capture std::func") << Benchmark::getEndString() << endl;
        }
        {
            FastFunc<int(int&, int)> t2 = rawFunc;
            FastFunc<void(int)> t1 = [&tempVar, &t2](int i)
            {
                tempVar = t2(tempVar, i);
            };
            Benchmark::start();
            ...
            lo("capture don_delegate") << Benchmark::getEndString() << endl;
        }
        lo() << endl;
    }
