...
#define NSUT_IMPL_GET_NAME_RUNNER(mName) \
    SSVPP_CAT(NSUT_IMPL_GET_NAME_TYPE(mName), runner)
#define NSUT_IMPL_GET_NAME_INSTANCE(mName) \
    SSVPP_CAT(NSUT_IMPL_GET_NAME_TYPE(mName), instance)
...
#define NSUT_IMPL_GENERATE_STRUCT(mName)                                  \
    ...
    {                                                                     \
        inline NSUT_IMPL_GET_NAME_TYPE(mName)()                           \
            : ::NSUnitTest::TestBase{SSVPP_TOSTR(mName),                  \
                  SSVPP_TOSTR(__LINE__), SSVPP_TOSTR(__FILE__)}           \
        {                                                                 \
        }                                                                 \
...
#define NSUT_IMPL_GENERATE_RUNNER(mName)                                \
    static ::NSUnitTest::Runner NSUT_IMPL_GET_NAME_RUNNER(mName){[]     \
        {                                                               \
            if(::NSUnitTest::wasTestExecuted(NSUT_IMPL_GET_KEY(mName))) \
                return;                                                 \
            ::NSUnitTest::setTestExecuted(NSUT_IMPL_GET_KEY(mName));    \
            ssvu::getEmplaceUptr<NSUT_IMPL_GET_NAME_TYPE(mName)>(       \
                ::NSUnitTest::getTestStorage(),                         \
                NSUT_IMPL_GET_NAME_INSTANCE(mName));                    \
        }};
...
#else
#define NSUT_TEST(mName) \
    inline void SSVPP_CAT(NSUT_IMPL_GET_NAME_TYPE(mName), unused)()
#define NSUT_EXPECT(...)
#define NSUT_RUN_ALL_TESTS() \
    while(false)             \
    {                        \
    }
#endif
...
namespace NSUnitTest
{
    ...
    public:
        inline TestBase(std::string mName, std::string mLine, std::string mFile)
            : name{std::move(mName)}, line{std::move(mLine)},
              file{std::move(mFile)}
        {
        }
    ...
    struct Fail
    {
        ...
        inline Fail(const TestBase* mTest, std::string mExpr)
            : test{mTest}, expr{std::move(mExpr)}
        {
        }
    };
    ...
    inline TestStorage& getTestStorage() noexcept
    {
        static TestStorage result;
        return result;
    }
    inline TestExecMap& getTestExecMap() noexcept
    {
        static TestExecMap result;
        return result;
    }

    inline bool wasTestExecuted(const std::string& mKey) noexcept
    {
        return getTestExecMap()[mKey];
    }
    inline void setTestExecuted(const std::string& mKey) noexcept
    {
