...
template<std::size_t TB> struct CBit : std::integral_constant<std::size_t, TB> { };
...
// shared
inline auto getLastIdx() noexcept { static auto lastIdx(0u); return lastIdx++; }
// ___

// templates1
template<typename T> inline auto getTypeId() noexcept { static auto idx(getLastIdx()); return idx; }
// ___
...
struct TypeIdManager
{
    ...
    template<typename T> inline static std::size_t get() noexcept { static std::size_t id{lastId++}; return id; }
};
