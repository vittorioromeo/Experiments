...
namespace ssvu
{
    namespace Internal
    {
        ...
        struct CTMax;
        template <SizeT TV0>
        struct CTMax<TV0> : IntegralConstant<SizeT, TV0>
        {
        };
        template <SizeT TV0, SizeT TV1, SizeT... TVs>
            struct CTMax<TV0, TV1, TVs...> : CTMax <
                                             TV0<TV1 ? TV1 : TV0, TVs...>
        {
        };

        template <typename... TArgs>
        inline constexpr auto getCTMaxSize() noexcept
        {
            return CTMax<sizeof(TArgs)...>::value;
        }
        template <typename... TArgs>
        inline constexpr auto getCTMaxAlign() noexcept
        {
            return CTMax<alignof(TArgs)...>::value;
        }
    }
    ...
    public:
        ...
        inline void init(TArgs&&... mArgs) noexcept(
            isNothrowConstructible<T, TArgs...>())
        {
...
#define EXP(...)                                           \
    if(!(__VA_ARGS__))                                     \
    {                                                      \
        lo() << #__VA_ARGS__ << " NOT TRUE!" << std::endl; \
    }                                                      \
    do                                                     \
    {                                                      \
    } while(false)
