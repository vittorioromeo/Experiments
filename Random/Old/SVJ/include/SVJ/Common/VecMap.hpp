    ...
    namespace Json
    {
        namespace Internal
        {
            ...
            private:
                ...
                template <typename T>
                inline static auto lookupHelper(
                    T& mVecMap, const TK& mKey) noexcept
                {
                    return lowerBound(mVecMap.data, mKey,
                ...
                inline auto lookup(const TK& mKey) noexcept
                {
                    return lookupHelper(*this, mKey);
                }
                inline auto lookup(const TK& mKey) const noexcept
                {
                    return lookupHelper(*this, mKey);
                }

                template <typename T>
                inline bool is(const T& mItr, const TK& mKey) const noexcept
                {
            ...
            public:
                ...
                inline VecMap(std::initializer_list<Item>&& mIL)
                {
                    ...
                    sort(data, [](const auto& mA, const auto& mB)
                        {
                ...
                inline auto& operator[](const TK& mKey)
                {
                    ...
                    return is(itr, mKey)
                               ? itr->second
                               : data.emplace(itr, mKey, TV{})->second;
                }
                ...
                inline auto operator==(const VecMap& mVM) const noexcept
                {
                    return data == mVM.data;
                }
                inline auto operator!=(const VecMap& mVM) const noexcept
                {
                    return !(operator==(mVM));
