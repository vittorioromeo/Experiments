        ...
        namespace Internal
        {
            ...
            private:
                ...
                inline auto getErrorSrc()
                {
                    ...
                    auto strMarked(
                        std::string{std::begin(src) + iStart,
                            std::begin(src) + iDStart} +
                        " >>>>>> " + std::string{std::begin(src) + iDStart,
                                         std::begin(src) + iDEnd} +
                        " <<<<<< " + std::string{std::begin(src) + iDEnd,
                                         std::begin(src) + iEnd});

                    auto strUnmarked(std::string{
                        std::begin(src) + iStart, std::begin(src) + iEnd});
                ...
                inline static constexpr auto isWhitespace(char mC) noexcept
                {
                    return mC == ' ' || mC == '\t' || mC == '\r' || mC == '\n';
                }
                inline static constexpr auto isNumberStart(char mC) noexcept
                {
                    return mC == '-' || isDigit(mC);
                }

                inline char getC() const noexcept { return src[idx]; }
                inline auto isC(char mC) const noexcept { return getC() == mC; }
                inline auto isCDigit() const noexcept
                {
                    return isDigit(getC());
                }
                ...
                inline void match(const char(&mKeyword)[TS])
                {
                    for(auto i(0u); i < TS - 1; ++i)
                    {
                        if(getC() != mKeyword[i])
                            throw ReadException("Invalid keyword",
                                std::string{"Couldn't match keyword `"} +
                                    std::string{mKeyword} + "'",
                                getErrorSrc());
                        ++idx;
                    }
                ...
                inline auto parseNull()
                {
                    match("null");
                    return Value{Null{}};
                }
                inline auto parseBoolFalse()
                {
                    match("false");
                    return Value{false};
                }
                inline auto parseBoolTrue()
                {
                    match("true");
                    return Value{true};
                }
                ...
                inline auto parseNumber()
                {
                    ...
                    auto realN(static_cast<Number::Real>(
                        std::strtod(src.data() + idx, &endChar)));
                ...
                inline auto parseArray()
                {
                    ...
                    while(true)
                    {
                        ...
                        throw ReadException{"Invalid array",
                            "Expected either `,` or `]`, got `"s + getC() + "`",
                            getErrorSrc()};
                    }
                ...
                inline auto parseObject()
                {
                    ...
                    while(true)
                    {
                        ...
                        skipWhitespaceAndComments();
                        if(!isC('"'))
                            throw ReadException{"Invalid object",
                                "Expected `\"` , got `"s + getC() + "`",
                                getErrorSrc()};
                        auto key(readString());
            ...
            public:
                ...
                inline Value parseValue()
                {
                    ...
                    throw ReadException{"Invalid value",
                        "No match for values beginning with `"s + getC() + "`",
                        getErrorSrc()};
                }
            };
        }
