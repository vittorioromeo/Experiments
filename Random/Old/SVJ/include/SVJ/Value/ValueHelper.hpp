...
#define SVJ_DEFINE_VALUEHELPER_NUMBER_IMPL(mType)          \
    template <>                                            \
    struct ValueHelper<mType>                              \
    {                                                      \
        inline static void set(Value& mV, const mType& mX) \
        {                                                  \
            mV.setNumber(Number{mX});                      \
        }                                                  \
        inline static decltype(auto) get(const Value& mV)  \
        {                                                  \
            return mV.getNumber().get<mType>();            \
        }                                                  \
        inline static auto is(const Value& mV) noexcept    \
        {                                                  \
            return mV.getType() == Value::Type::Number;    \
        }                                                  \
    };
...
#define SVJ_DEFINE_VALUEHELPER_BIG_MUTABLE(mType)                    \
    ...
    {                                                                \
        template <typename T>                                        \
        inline static void set(Value& mV, T&& mX)                    \
        {                                                            \
            SSVPP_CAT(mV.set, mType)(fwd<T>(mX));                    \
        }                                                            \
        template <typename T>                                        \
        inline static decltype(auto) get(T&& mV)                     \
        {                                                            \
            return SSVPP_CAT(fwd<T>(mV).get, mType)();               \
        }                                                            \
        inline static auto is(const Value& mV) noexcept              \
        {                                                            \
            return mV.getType() == SSVPP_EXPAND(Value::Type::mType); \
        }                                                            \
    };
...
#define SVJ_DEFINE_VALUEHELPER_SMALL_IMMUTABLE(mType)                \
    ...
    {                                                                \
        inline static void set(Value& mV, const mType& mX)           \
        {                                                            \
            SSVPP_CAT(mV.set, mType)(mX);                            \
        }                                                            \
        inline static decltype(auto) get(const Value& mV)            \
        {                                                            \
            return SSVPP_CAT(mV.get, mType)();                       \
        }                                                            \
        inline static auto is(const Value& mV) noexcept              \
        {                                                            \
            return mV.getType() == SSVPP_EXPAND(Value::Type::mType); \
...
#undef SVJ_DEFINE_VALUEHELPER_SMALL_IMMUTABLE
            ...
            struct ValueHelper<char[TS]>
            {
                inline static void set(Value& mV, const char(&mX)[TS])
                {
                    mV.setString(mX);
                }
                inline static auto is(const Value& mV) noexcept
                {
                    return mV.getType() == Value::Type::String;
                }
            };
        }
    }
