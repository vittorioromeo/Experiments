...
namespace ssvu
{
    namespace Json
    {
        ...
        public: // private:
            ...
            template <typename T>
            inline void setObject(T&& mX)
            {
                type = Type::Object;
                new(&h.hObject) Object(fwd<T>(mX));
            }
            template <typename T>
            inline void setArray(T&& mX)
            {
                type = Type::Array;
                new(&h.hArray) Array(fwd<T>(mX));
            }
            template <typename T>
            inline void setString(T&& mX)
            {
                type = Type::String;
                new(&h.hString) String(fwd<T>(mX));
            }
            inline void setNumber(const Number& mX) noexcept
            {
                type = Type::Number;
                h.hNumber = mX;
            }
            inline void setBool(Bool mX) noexcept
            {
                type = Type::Bool;
                h.hBool = mX;
            }
            ...
            inline auto& getObject() & noexcept
            {
                SSVU_ASSERT(is<Object>());
                return h.hObject;
            }
            inline auto& getArray() & noexcept
            {
                SSVU_ASSERT(is<Array>());
                return h.hArray;
            }
            inline auto& getString() & noexcept
            {
                SSVU_ASSERT(is<String>());
                return h.hString;
            }
            inline const auto& getObject() const& noexcept
            {
                SSVU_ASSERT(is<Object>());
                return h.hObject;
            }
            inline const auto& getArray() const& noexcept
            {
                SSVU_ASSERT(is<Array>());
                return h.hArray;
            }
            inline const auto& getString() const& noexcept
            {
                SSVU_ASSERT(is<String>());
                return h.hString;
            }
            ...
            {
                SSVU_ASSERT(is<Object>());
                return std::move(h.hObject);
            }
            inline auto getArray() && noexcept
            {
                SSVU_ASSERT(is<Array>());
                return std::move(h.hArray);
            }
            inline auto getString() && noexcept
            {
                SSVU_ASSERT(is<String>());
                return std::move(h.hString);
            }

            inline auto getNumber() const noexcept
            {
                SSVU_ASSERT(is<Number>());
                return h.hNumber;
            }
            inline auto getBool() const noexcept
            {
                SSVU_ASSERT(is<Bool>());
                return h.hBool;
            }
            inline auto getNull() const noexcept
            {
                SSVU_ASSERT(is<Null>());
                return Null{};
            }
        ...
        public:
            ...
            template <typename T, SVJ_ENABLE_IF_IS_NOT(T, Value)>
            inline Value(T&& mX)
            {
                set(fwd<T>(mX));
            }
            ...
            template <typename T>
            inline void set(T&& mX)
            {
                deinitCurrent();
                Internal::ValueHelper<RemoveAll<T>>::set(*this, fwd<T>(mX));
            }
            ...
            {
                return Internal::ValueHelper<T>::get(*this);
            }
            template <typename T>
            ...
            template <typename T>
                decltype(auto) get() && noexcept
            {
                return Internal::ValueHelper<T>::get(std::move(*this));
            }

            // "Implicit" Value from Object by Key getters
            inline auto& operator[](const Key& mKey)
            {
                return getObject()[mKey];
            }
            inline const auto& operator[](const Key& mKey) const
            {
                return getObject().at(mKey);
            }
            ...
            {
                return Internal::ValueHelper<T>::is(*this);
            }
            ...
            inline auto operator!=(const Value& mV) const noexcept
            {
                return !(operator==(mV));
            }

            template <WriterMode TWS = WriterMode::Pretty, bool TFmt = false>
            void writeToStream(std::ostream& mStream) const;
            template <WriterMode TWS = WriterMode::Pretty, bool TFmt = false>
            void writeToString(std::string& mStr) const;
            template <WriterMode TWS = WriterMode::Pretty, bool TFmt = false>
            void writeToFile(const ssvufs::Path& mPath) const;
            template <WriterMode TWS = WriterMode::Pretty, bool TFmt = false>
            auto getWriteToString() const;

            void readFromString(std::string mStr);
            void readFromFile(const ssvufs::Path& mPath);

            static inline Value fromString(const std::string& mStr)
            {
                Value result;
                result.readFromString(mStr);
                return result;
            }
            static inline Value fromFile(const ssvufs::Path& mPath)
            {
                Value result;
