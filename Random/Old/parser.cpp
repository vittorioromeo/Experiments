...
namespace Engine
{
    template <typename TT, typename TG, typename TTD>
    struct Language
    {
        using Tkn = TT;
        using Gmr = TG;
    ...
    using TknT = typename TL::Tkn;
    template <typename TL>
    using GmrT = typename TL::Gmr;
    template <typename TL>
    using TknDataT = typename TL::TknData;
    ...
    public:
        ...
        T& emplaceChild(TArgs&&... mArgs)
        {
            return ssvu::getEmplaceUptr<T>(
                children, std::forward<TArgs>(mArgs)...);
        }
        ...
        template <typename T>
        T& getChildAs(std::size_t mIdx)
        {
            return reinterpret_cast<T&>(*children[mIdx]);
        }
    };
    ...
    public:
        ...
        inline TknT<TL> getTkn() const noexcept
        {
            assert(type == PsltType::Tkn);
            return tkn;
        }
        inline GmrT<TL> getGmr() const noexcept
        {
    ...
    public:
        ...
        inline GmrExpansion(const FuncType& mFunc, TArgs... mArgs) noexcept
            : func{mFunc}
        {
    ...
    public:
        ...
        template <typename... TArgs>
        inline Node& createChild(TArgs&&... mArgs)
        {
            auto& result(ssvu::getEmplaceUptr<Node>(
                children, std::forward<TArgs>(mArgs)...));
    ...
    private:
        ...
        inline void shift()
        {
            // Pop a token from tokenStack and push it on parseStack and
            // nodeStack
        ...
        inline bool parseStackMatchesExpansion(
            std::size_t mStartAt, const GmrExpansion<TL>& mGmrExpansion)
        {
            ...
            for(auto i(0u); i < expSize; ++i)
                if(mGmrExpansion.getParselets().at(i) !=
                    parseStack[mStartAt + i])
                    return false;
...
{
    Engine::GmrExpansion<Lang::LangSpec> gmr1{
        Lang::Gmr::Expr, Lang::Gmr::Expr, Lang::Gmr::Op, Lang::Gmr::Op};
    Engine::GmrExpansion<Lang::LangSpec> gmr2{
        Lang::Gmr::Expr, Lang::Tkn::POpen, Lang::Gmr::Op, Lang::Tkn::PClose};
...
namespace ssvu
{
    ...
    template <typename TL>
    struct Stringifier<Parselet<TL>>
    {
        template <bool TFmt>
        inline static void impl(
            std::ostream& mStream, const Parselet<TL>& mValue)
        {
            if(mValue.getType() == PsltType::Tkn)
            {
                Internal::callStringifyImpl<TFmt>(
                    mStream, Engine::TknT<TL>(mValue.getTkn()));
            }
            else if(mValue.getType() == PsltType::Gmr)
            {
                Internal::callStringifyImpl<TFmt>(
                    mStream, Engine::GmrT<TL>(mValue.getGmr()));
            }
        }
    };
    template <typename TL>
    struct Stringifier<Node<TL>>
    {
        template <bool TFmt>
        inline static void printNode(
            std::ostream& mStream, const Node<TL>& mValue, int mDepth)
        {
            ...
            {
                Stringifier<Node<TL>>::printNode<TFmt>(
        ...
        template <bool TFmt>
        inline static void impl(std::ostream& mStream, const Node<TL>& mValue)
        {
            Internal::callStringifyImpl<TFmt>(mStream, "\n\n");
...
namespace Lang
{
    ...
    {
        int value;
        inline Value operator+(const Value& mOther) const
        {
            return Value{value + mOther.value};
        }
    };
    ...
    {
    public:
        inline BinaryAdditionExpr inline Value evaluate() override
        {
...
int main()
{
    ...
    std::vector<Tkn> tkns{Tkn::POpen, Tkn::Num, Tkn::OpAdd, Tkn::POpen,
        Tkn::Num, Tkn::PClose, Tkn::PClose, Tkn::OpAdd, Tkn::Num, Tkn::OpAdd,
        Tkn::POpen, Tkn::Num, Tkn::PClose};
    ...
    */
    ...
    parser.createRule(
        []
        { return std::make_unique<BinaryAdditionExpr>(
        },
        Gmr::Expr, Gmr::Op, Gmr::Expr);
