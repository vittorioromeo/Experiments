...
struct LambdaHack
{
    ...
    void initData(T&& arg)
    {
        new(&storage) L(FWD(arg));
    }
    template <typename LL>
    inline static R callTarget(Storage& ms, Args... args)
    {
        return ssvu::castStorage<LL>(ms)(args...);
    }

    template <typename LL>
    LambdaHack(Storage& ms, LL&&)
        : storage(ms), target(callTarget<LL>)
    {
    }
    template <typename... TArgs>
    inline R operator()(TArgs&&... args)
    {
        return target(storage, FWD(args)...);
    }
};
...
struct BenchmarkGroupRAII
{
    ...
    inline BenchmarkGroupRAII(const std::string& mGroupName)
        : groupName{mGroupName}
    {
        ssvu::Benchmark::groupReset(groupName);
    }
    inline ~BenchmarkGroupRAII() { ssvu::Benchmark::groupEndLo(groupName); }
};
...
int main()
{
    ...
    auto l0 = [&captured](int x)
    {
        state = !state;
        captured += x;
    };
    auto l1 = [&captured](int x)
    {
        state = !state;
        captured -= x;
    };
    auto l2 = [&captured](int x)
    {
        state = !state;
        captured = x + 1;
    };
