...
struct pool { template <typename TF> void post(TF&& f){ f(); } };
...
int main()
{
    ...
    latch l{1};
    l.execute_and_wait_until_zero([&l]{
        my_pool.post([&l]{ 
            op0(); op0(); op0(); op0(); 
            l.decrement_and_notify_one(); 
        });
    });

#else

    my_context ctx{my_pool};
    ctx.build([]{ op0(); }).then([]{ op0(); }).then([]{ op0(); }).then([]{ op0(); }).wait();
