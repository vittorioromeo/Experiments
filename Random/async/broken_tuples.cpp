...
#undef BOUND_F
                       ...
                       TF
    {
        ...
        auto execute(TTuple&& x) &
        {
            this->ctx()._p.post([ this, x = FWD(x) ]() mutable {
                std::experimental::apply(
                    [this](auto&&... args) {
                        with_void_to_nothing(as_f(), FWD(args)...);
                    },
                    FWD(x));
            });
        }

        template <typename TTuple, typename TNode, typename... TNodes>
        auto execute(TTuple&& x, TNode& n, TNodes&... ns) &
        {
            this->ctx()._p.post([ this, x = FWD(x), &n, &ns... ]() mutable {

                decltype(auto) res = std::experimental::apply(
                    [this](auto&&... args) -> decltype(auto) {
                        return with_void_to_nothing(as_f(), FWD(args)...);
                    },
                    FWD(x));

                this->ctx()._p.post(
                    [ res = std::move(res), &n, &ns... ]() mutable {
                        n.execute(std::make_tuple(std::move(res)), ns...);
                    });
            });
        }
                           ...
                           TFs...
    {
        ...
        using input_type = void_to_nothing_t<typename TParent::return_type>;
        using return_type = std::tuple<void_to_nothing_t<decltype(call_ignoring_nothing(
        ...
        auto execute(TTuple&&x, TNode& n, TNodes&... ns) &
        {
            ...
            auto exec = [this, &x, &n, &ns...](auto& f) {
                this->ctx()._p.post([&] {
                    std::experimental::apply(
                        [this, &f](auto&&... args) -> decltype(auto) {
                            return with_void_to_nothing(f, FWD(args)...);
                        },
                        FWD(x));
                    if(--_ctr == 0)
                    {
...
/*
    ...
    ctx.build([] { return 10; })
        .then([](int) { ll::print_sleep_ms(100, "A"); },
            [](int) { ll::print_sleep_ms(100, "B"); },
            [](int) { ll::print_sleep_ms(100, "C"); })
        .then([] { ll::print_sleep_ms(1, "done"); })
        .start();
*/
    ctx.build([] { return 10; })
        .then([](int) { return 1; }, [](int) { return 'a'; })
        .then([](int, char) { ll::print_sleep_ms(1, "done"); })
        .start();
