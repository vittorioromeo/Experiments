...
namespace Boilerplate
{
    ...
    private:
        inline void initInput()
        {
            ...
            gs.addInput({{IK::Escape}}, [this](ssvu::FT)
                {
                    stop();
                });

            gs.addInput({{IK::A}}, [this](ssvu::FT)
                {
                    gCamera.pan(-4, 0);
                });
            gs.addInput({{IK::D}}, [this](ssvu::FT)
                {
                    gCamera.pan(4, 0);
                });
            gs.addInput({{IK::W}}, [this](ssvu::FT)
                {
                    gCamera.pan(0, -4);
                });
            gs.addInput({{IK::S}}, [this](ssvu::FT)
                {
                    gCamera.pan(0, 4);
                });
            gs.addInput({{IK::Q}}, [this](ssvu::FT)
                {
                    gCamera.zoomOut(1.1f);
                });
            gs.addInput({{IK::E}}, [this](ssvu::FT)
                {
                    gCamera.zoomIn(1.1f);
                });
        }
    ...
    public:
        inline TestApp(ssvs::GameWindow& mGW) : App{mGW}
        {
            ...
            gState.onUpdate += [this](ssvu::FT mFT)
            {
                update(mFT);
            };
            gState.onDraw += [this]
...
namespace Trig
{
    template <ssvu::SizeT TPrecision>
    struct TrigTable
    {
    private:
...
namespace Batch
{
    ...
    namespace Impl
    {
        ...
        struct TextureHandle
        {
            ...
            inline TextureHandle(ssvu::SizeT mID, float mSizeX,
                float mSizeY) noexcept : halfSize{mSizeX / 2.f, mSizeY / 2.f},
                                         nw{0, 0},
                                         ne{mSizeX, 0},
                                         sw{0, mSizeY},
                                         se{mSizeX, mSizeY},
        ...
        public:
            inline void setTexture(const Impl::TextureHandle& mX) noexcept
            {
                hTexture = mX;
            }
            inline void setPosition(const ssvs::Vec2f& mX) noexcept
            {
                position = mX;
            }
            inline void setOrigin(const ssvs::Vec2f& mX) noexcept
            {
                origin = mX;
            }
            ...
            inline const auto& getTextureHalfSize() const noexcept
            {
                return hTexture.halfSize;
            }
        ...
        protected:
            inline auto getRotatedPoint(float mX, float mY, float mSSin,
                float mSCos, const ssvs::Vec2f& mTOrigin) const noexcept
            {
                return ssvs::Vec2f{(mTOrigin.x - mX) * mSCos -
                                       (mTOrigin.y - mY) * mSSin + position.x,
                    (mTOrigin.y - mY) * mSCos + (mTOrigin.x - mX) * mSSin +
                        position.y};
            }
        };
    }
    ...
    public:
        inline LSprite(const Impl::TextureHandle& mTH,
            const Impl::LayerHandle& mLH) noexcept : Impl::SpriteBase{mTH},
                                                     hLayer(mLH)
        {
        }

        inline void setLayer(const Impl::LayerHandle& mX) noexcept
        {
            hLayer = mX;
        }
    ...
    private:
        inline void emplaceRotatedVertex(Impl::VectorQuads& mV, float mX,
            float mY, float mSSin, float mSCos, const ssvs::Vec2f& mTOrigin,
            const ssvs::Vec2f& mTPos) const
        {
            mV.emplace_back(
                getRotatedPoint(mX, mY, mSSin, mSCos, mTOrigin), mTPos);
        }
        ...
        inline void emplaceVertices(Impl::VectorQuads& mV) const
        {
            ...
            auto erv([this, &mV, &tOrigin, &sSin, &sCos](
                auto mX, auto mY, const auto& mTp)
                {
    ...
    public:
        inline ZSprite(const Impl::TextureHandle& mTH,
            const Impl::ZLayerHandle& mZLH, Impl::ZOrder mZOrder = 0) noexcept
            : Impl::SpriteBase{mTH},
              hZLayer(mZLH),
              zOrder{mZOrder}
        {
        ...
        inline void setZLayer(const Impl::ZLayerHandle& mX) noexcept
        {
            hZLayer = mX;
        }
    ...
    private:
        inline void emplaceVertices(Impl::ZData& mZD) const
        {
            ...
            auto erv([this, &mZD, &tOrigin, &sSin, &sCos](
                auto mI, auto mX, auto mY, const auto& mTp)
                {
    ...
    private:
        inline void clearLayers() noexcept
        {
    ...
    public:
        inline auto bind(const sf::Texture& mX)
        {
            ...
            return Impl::TextureHandle{
                next - 1, ssvu::toFloat(ts.x), ssvu::toFloat(ts.y)};
        }
    ...
    inline void Impl::Layer::drawOn(
        Manager& mManager, sf::RenderTarget& mX) noexcept
    {
        ...
        {
            auto& v(vVectors[i]);
            mX.draw(v.data(), v.size(), sf::PrimitiveType::Quads,
                sf::RenderStates{mManager.boundTextures[i]});
        }
    }

    inline void Impl::ZLayer::drawOn(
        Batch::Manager& mMgr, sf::RenderTarget& mX) noexcept
    {
        ...
        while(curr != end)
        {
            ...
            mX.draw(vs.getDataPtr(), cv - vs.getDataPtr(),
                sf::PrimitiveType::Quads,
                sf::RenderStates{mMgr.boundTextures[tx]});
        }

        zvs.clear();
    }

    /*template<typename... TArgs> inline Impl::LayerHandle::mkSprite(TArgs&&...
    mArgs)
    {
        return Batch::LSprite(const Impl::TextureHandle &mTH, const
    Impl::LayerHandle &mLH)
    }*/
}
...
struct MovingThing
{
    ...
    inline MovingThing(const decltype(spr)& mSpr) : spr{mSpr}
    {
        ...
        spr.setOrigin(
            ssvs::Vec2f{ssvu::getRndR(0, hts.x), ssvu::getRndR(0, hts.y)});
    ...
    inline void draw(Batch::Manager& mMgr) { mMgr.enqueue(spr); }
};
...
int main()
{
    ...
    {
        MovingThing l{Batch::ZSprite{
            btxs[ssvu::getRndI(0, btxs.size())], btzlTest, i % 100}};
        l.spr.setPosition(
            ssvs::Vec2f{ssvu::getRndR(0.f, 800.f), ssvu::getRndR(0.f, 600.f)});
        auto r(ssvu::getRndR(0.f, ssvu::tau));
        l.velocity = ssvs::getVecFromRad(
            ssvu::getRndR(0.f, ssvu::tau), ssvu::getRndR(0.1f, 20.5f));
