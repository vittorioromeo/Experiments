...
namespace ssvut
{
    namespace Impl
    {
        ...
        class GraphLink : public TGraph::StorageLinkBase
        {
        private:
        ...
        template <typename TGraph>
        class GraphNode : public TGraph::StorageNodeBase
        {
        private:
            using NodePtr = typename TGraph::NodePtr;
        ...
        public:
            ...
            inline const decltype(TGraph::StorageNodeBase::links)&
            getLinks() const
            {
                return TGraph::StorageNodeBase::links;
            }
            inline bool isIsolated() const noexcept
            {
                return TGraph::StorageNodeBase::links.empty();
            }
        };

        template <typename TGraph>
        struct GraphStorageFreeStore
        {
            using NodeDerived = typename TGraph::NodeDerived;
            ...
            struct NodeBase
            {
                ...
                inline static NodePtr getNodePtr(
                    GraphNode<TGraph>* mNode) noexcept
                {
                    return ssvu::castUp<NodeDerived>(mNode);
                }
            };
            struct LinkBase
            {
            ...
            inline static const NodePtr& getNodeNull() noexcept
            {
                static NodePtr result{nullptr};
                return result;
            }
            inline static constexpr bool isNodeValid(
                const NodePtr& mNode) noexcept
            {
                return mNode != getNodeNull();
            }
    ...
    template <typename TNode, typename TLink,
        template <typename> class TStorage = Impl::GraphStorageFreeStore>
    class Graph
    {
    public:
    ...
    public:
        inline const decltype(nodes)& getNodes() const noexcept
        {
            return nodes;
        }
        ...
        inline static const NodePtr& getNodeNull() noexcept
        {
            return Storage::getNodeNull();
        }
        inline static constexpr bool isNodeValid(NodePtr mNode) noexcept
        {
            return Storage::isNodeValid(mNode);
        }
