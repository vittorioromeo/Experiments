...
namespace ssvvm
{
    template <bool TStart>
    inline bool isIdnfChar(char mChar) noexcept
    {
        static constexpr char validChars[]{'_'};
        return (TStart && ssvu::isAlphabetical(mChar)) ||
               (!TStart && ssvu::isAlphanumeric(mChar)) ||
               ssvu::contains(validChars, mChar);
    }
    ...
    inline ASMLexicalAnalyzer& getASMLA() noexcept
    {
        ...
        if(!initialized)
        {
            ...
            la.createMatchFSM(VMToken::Float)
                .matchRepeat(&ssvu::isDigit, FSMNT::NonTerminal)
                .once(".f", FSMNT::Terminal);
            la.createMatchFSM(VMToken::Integer)
                .matchRepeat(&ssvu::isDigit, FSMNT::Terminal);
            la.createMatchFSM(VMToken::Identifier)
                .matchRepeat(&isIdnfChar<true>, FSMNT::Terminal)
                .matchRepeat(&isIdnfChar<false>, FSMNT::Terminal);
            la.createMatchFSM(VMToken::Comment)
                .once("//", FSMNT::Terminal)
                .matchAnythingUntilOnce(
                    FSMNT::NonTerminal, "\n", FSMNT::Terminal);
            la.createMatchFSM(VMToken::WhiteSpace)
                .matchRepeat(&ssvu::isSpace, FSMNT::Terminal);
