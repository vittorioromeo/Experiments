...
namespace ssvvm
{
    ...
    public:
        inline Register& get(std::size_t mIdx) noexcept
        {
            SSVU_ASSERT(mIdx < TSize);
            return registers[mIdx];
        }
        inline const Register& get(std::size_t mIdx) const noexcept
        {
            SSVU_ASSERT(mIdx < TSize);
            return registers[mIdx];
        }
        inline Value& getValue(std::size_t mIdx) noexcept
        {
