...
namespace avz
{
    ...
    private:
        ...
        template <typename T>
        inline void render(T& mX)
        {
            gameWindow->draw(mX);
        }
        template <typename... TArgs>
        inline auto& createTA(TArgs&&... mArgs)
        {
            return taRecycler.getCreateEmplace(
                getCurrentTACtx().taQueue, ssvu::fwd<TArgs>(mArgs)...);
        }

        inline void simultaneously() { getCurrentTACtx().simultaneously(); }


        inline void makeTACtx()
        {
            taCtxStack.emplace_back(ssvu::makeUPtr<Internal::TACtx>());
        }
        inline void restoreTACtx() { ++toPop; }

    public:
        inline Ctx(ssvs::GameWindow& mGameWindow)
            : root{*this}, gameWindow{&mGameWindow}
        {
        ...
        inline void skipAnim() { getCurrentTACtx().skipAnim(); }
        ...
        inline void update(FT mFT)
        {
            ...
            {
                taCtxStack.pop_back();
                ssvu::lo("BACK TO CONTEXT") << taCtxStack.size() - 1 << "\n";
            }
