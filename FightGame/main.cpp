...
{
    auto& e(createEntity());
    auto& cPhys(e.createComponent<FGCPhys>(
        game, true, ssvs::getCenter(mMin, mMax), ssvs::getSize(mMin, mMax)));
    cPhys.getBody().addGroups(FGGroup::FGGSolid);
    auto& cPhysRender(e.createComponent<FGCPhysRender>(game));
    return e;
}

inline Entity& FGFactory::createTestEntity(
    const Vec2i& mPos, const Vec2i& mSize, const Vec2f& mVel, bool mPlayer)
{
    ...
    auto& cFighterAnim(
        e.createComponent<FGCFighterAnim>(game, FGAssets::get().tsFighter));
...
inline Entity& FGFactory::createProjPunch(Body& mParent, const Vec2i& mPos,
