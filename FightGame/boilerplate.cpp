...
template <typename T>
inline constexpr float toPixels(T mValue)
{
    return mValue / 100.f;
}
template <typename T>
inline constexpr int toCoords(T mValue)
{
    return mValue * 100;
}
template <typename T>
...
public:
    ...
    inline void update(FT mFT) override { integrate(mFT); }
    ...
    inline Vec2f getHalfSizePx() const noexcept
    {
        return toPixels(getHalfSize());
    }
...
public:
    ...
    inline void init() { cPhys = &getEntity().getComponent<FGCPhys>(); }
...
public:
    ...
    inline auto& createTestEntity(
        const Vec2i& mPos, const Vec2i& mSize, const Vec2f& mVel)
    {
        auto& e(createEntity());
        auto& cPhys(e.createComponent<FGCPhys>(game, mPos, mSize));
...
inline void FGCPhysRender::draw() { game.render(shp); }
