...
class FGGameInput
{
	...
	public:
		inline FGGameInput(FGGame& mGame) noexcept : game{mGame}
		{
			initInput();
		}
...
inline void FGGameInput::initInput()
{
	...
	gs.addInput({{IK::Escape}}, [this](FT){ game.getGameWindow().stop(); });	

	gs.addInput({{IK::A}}, [this](FT){ game.getGameCamera().pan(-4, 0); });
	gs.addInput({{IK::D}}, [this](FT){ game.getGameCamera().pan(4, 0); });
	gs.addInput({{IK::W}}, [this](FT){ game.getGameCamera().pan(0, -4); });
	gs.addInput({{IK::S}}, [this](FT){ game.getGameCamera().pan(0, 4); });
	gs.addInput({{IK::Q}}, [this](FT){ game.getGameCamera().zoomOut(1.1f); });
	gs.addInput({{IK::E}}, [this](FT){ game.getGameCamera().zoomIn(1.1f); });
